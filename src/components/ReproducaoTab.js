...
const ReproducaoTab = () => {
  ...
  const loadDemoData = () => {
    ...
    setActiveProtocols([
      { id: 1, animalId: 2235, animalTag: '2235', animalNickname: 'Estrela', protocolName: 'P4/E2 8 Dias', startDate: '2025-01-15', currentDay: 8, iatfDate: '2025-01-25', status: 'active' },
      { id: 2, animalId: 8888, animalTag: '8888', animalNickname: '', protocolName: '7 Dias Otimizado', startDate: '2025-01-18', currentDay: 3, iatfDate: '2025-01-27', status: 'active' },
    ]);

    setUpcomingBirths([
      { id: 1, animalTag: '5430', animalNickname: 'Mimosa', projectedBirthDate: '2025-01-25', daysUntilBirth: 4 },
      { id: 2, animalTag: '3421', animalNickname: 'Boneca', projectedBirthDate: '2025-02-02', daysUntilBirth: 12 },
    ]);

    setProtocols([
      { id: 1, name: 'P4/E2 Padrão de 8 Dias (Gado de Corte)', protocolType: 'gado_de_corte', durationDays: 10 },
      { id: 2, name: 'Protocolo de Curta Duração de 7 Dias', protocolType: 'corte_otimizado', durationDays: 9 },
      { id: 3, name: 'Protocolo Ovsynch Modificado (Gado de Leite)', protocolType: 'gado_de_leite', durationDays: 10 },
    ]);

    setNotes([
      { id: 1, createdBy: 'Dr. Silva', role: 'vet', createdAt: '2025-01-15T10:30:00', noteText: 'Dispositivo inserido sem complicações. Vaca em bom estado corporal (ECC 3,5).' },
      { id: 2, createdBy: 'João', role: 'rancher', createdAt: '2025-01-18T08:15:00', noteText: 'Vaca apresentando cio aparente. Protocolo seguindo conforme esperado.' }
    ]);
  };
  ...
  const handleCSVImport = (e) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        const importData = results.data.filter(row => row.brinco || row.ear_tag);
        
        try {
          const token = localStorage.getItem('zapmanejo_token');
          const res = await fetch('/api/reproduction/import-csv', {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({ animals: importData })
          });

          if (res.ok) {
            const result = await res.json();
            alert(t('reproduction.csv_import_success', { count: result.imported }));
            fetchDashboardData();
          }
        } catch (err) {
          console.error('CSV import failed:', err);
          alert(t('reproduction.csv_import_error'));
        }
      }
    });
